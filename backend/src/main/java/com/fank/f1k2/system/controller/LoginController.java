...
public class LoginController {
    ...
    public F1k2Response login(
        ...
        StaffInfo userInfo1 = staffInfoService.getOne(Wrappers.<StaffInfo>lambdaQuery().eq(StaffInfo::getUserId, user.getUserId()));
        if (userInfo1 != null) {
    ...
    public void regist(
            @NotBlank(message = "{required}") String username,
            @NotBlank(message = "{required}") String password, @NotBlank(message = "{required}") String email, @NotBlank(message = "{required}") String name) throws Exception {
        this.userService.regist(username, password, email, name);
    }
