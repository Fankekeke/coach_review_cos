...
<template>
  <div class="page-container" style="width: 100%;">
    <a-row :gutter="16" class="content-wrapper">
      <!-- 左侧问题列表 -->
      <a-col :span="12" class="questions-list" :offset="6">
        <a-card
          :loading="loading"
          :bordered="false"
        ...
        >
          <div slot="title" class="card-header">
            ...
            <span class="header-title">学生问答</span>
    ...
    >
      <div class="detail-content" v-if="detailModal.data">
        ...
        <div class="answer-list">
          ...
          />
          <div>
            <a-divider v-if="detailModal.data.status !== '已采纳' || detailModal.data.sysUserId === (currentUser && currentUser.userId)">回答</a-divider>
            <div class="answer-input-wrapper" v-if="currentUser">
              <a-textarea
                v-model="answerContent"
                placeholder="请输入您的回答，帮助提问者解决问题..."
                :rows="4"
                :maxLength="2000"
                showCount
                class="answer-input"
...
export default {
  ...
  methods: {
    handleSubmitAnswer () {
      ...
      this.$post('/business/party-answers', {
        ...
        if (r.data.code === 0) {
          ...
          this.$message.error(r.data.msg || '回答提交失败')
        }
      }).catch((error) => {
    ...
    queryVotesByUser (questionId) {
      this.$get('/business/content-votes/queryVotesByUser', {
        userId: this.currentUser.userId,
        questionId
      }).then((r) => {
    ...
    handleAcceptAnswer () {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          this.$get('/business/party-questions/acceptAnswer', {
            questionId: this.detailModal.data.id
          }).then((r) => {
            if (r.data.code === 0) {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.$post('/business/party-questions', {
            userId: this.currentUser.userId,
            title: values.title,
    ...
    handleUpvote (answer) {
      ...
      this.$post('/business/content-votes', {
        answerId: answer.id,
        userId: this.currentUser.userId,
        voteType: 1
      }).then((r) => {
